pub fn naive(x1: &[f32], y1: &[f32], x2: &[f32], y2: &[f32], bins: &[f32]) -> std::vec::Vec<i32> {
    checks(x1, y1, x2, y2, bins);
...
fn checks(x1: &[f32], y1: &[f32], x2: &[f32], y2: &[f32], bins: &[f32]) {
...
mod test {
    ...
    fn basic_funcionality() {
        ...
        res = naive(&x, &y, &x, &y, &bins);
        assert_eq!(res, vec![2, 2]);

        let res = naive(&y, &x, &y, &x, &bins);
        assert_eq!(res, vec![2, 2]);
    }
}
